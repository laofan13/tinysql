...
func (c *RegionCache) GroupKeysByRegion(bo *Backoffer, keys [][]byte, filter func(key, regionStartKey []byte) bool) (map[RegionVerID][][]byte, RegionVerID, error) {
	...
	groupsKeys := make(map[RegionVerID][][]byte, 0)
	var firstVerID RegionVerID
	var lastLoc *KeyLocation

	for i, key := range keys {
		if lastLoc == nil || !lastLoc.Contains(key) {
			var err error
			lastLoc, err = c.LocateKey(bo, key)
			if err != nil {
				return nil, firstVerID, err
			}
			if filter != nil && filter(key, lastLoc.StartKey) {
				continue
			}
		}
		id := lastLoc.Region
		...
		groupsKeys[id] = append(groupsKeys[id], key)
	}
	return groupsKeys, firstVerID, nil
}
